...
def invoice2Excel(IN_PATH:str, OUT_PATH:str, WRITE_MODE:str):
...
def main():
    ...
    WRITE_MODE = 'write'
    # parse params
    opts, args = getopt.getopt(sys.argv[1:], 'p:ts:a', ['test', 'path=', 'save=', 'append'])
    for opt, arg in opts:
        if opt in ['-p', '--path']:
            IN_PATH = arg
        elif opt in ['--test', '-t']:
            ...
            WRITE_MODE = 'append'
    invoice2Excel(IN_PATH,OUT_PATH,WRITE_MODE)
